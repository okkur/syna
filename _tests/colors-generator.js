...
const fragments = fs
  ...
  .reduce((tmp, dir) => {
    ...
    const fragments = inside
      .filter(file => file.match(/\.md$/) && file.indexOf('code-') === -1 && file !== 'index.md')
      .reduce((tmp, file) => {
        tmp[file.replace('.md', '')] = `${paths.fragments}/${dir}/${file}`;
        return tmp;
      }, {})
    const nested = inside
      .filter(file => fs.lstatSync(`${paths.fragments}/${dir}/${file}`).isDirectory())
      .reduce((tmp, nDir) => {
...
+++`
  ...
  fs.writeFile(`${paths.devColors}/_index/index.md`, content, 'utf-8', () => {})
...
Object.keys(fragments).forEach((fragment, i) => {
  Object.keys(fragments[fragment].fragments).forEach(filename => {
    parseBlackFriday(fragment, fs.readFileSync(fragments[fragment].fragments[filename], 'utf-8'), filename)
  })
  Object.keys(fragments[fragment].nested).forEach(dir => {
    ...
    Object.keys(fragments[fragment].nested[dir]).forEach(filename => {
      ...
      backgrounds.forEach(background => {
        fs.createReadStream(fragments[fragment].nested[dir][filename]).pipe(fs.createWriteStream(`${paths.devColors}/${fragment}/${dir}-${background}/${filename}`));
      })
    })
  })
})
...
function parseBlackFriday(fragment, content, filename, dir) {
  if (!content.match(/background\s?=\s".*"/mi)) {
    return
  }
  ...
  if (dir && !fs.existsSync(`${paths.devColors}/${fragment}/${dir}`)) {
    backgrounds.forEach(background => {
      fs.mkdirSync(`${paths.devColors}/${fragment}/${dir}-${background}`)
    })
  }
  fs.writeFile(`${paths.devColors}/${fragment}/index.md`, indexTemplate.replace(/%fragment%/g, fragment), 'utf-8', () => {})

  backgrounds.forEach((background, i) => {
    const tmp = content
      .replace(/background\s?=\s".*"/mi, `background = "${background}"`)
      .replace(/weight\s?=\s?"?\d+"?/mi, `weight = ${110 + i * 10}`)
    fs.writeFile(`${paths.devColors}/${fragment + (dir ? `/${dir}-${background}` : '')}/${filename + (dir ? '' : `-${background}`)}.md`, tmp, 'utf-8', () => {})
  })
}

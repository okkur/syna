...
describe('jq-helpers', () => {
  ...
  it('should read and set innerHTML of selected element', () => {
    cy.window().then(win => {
      ...
      win.jqHelpers('#empty-content .content-body').html('<h3>Not empty now</h3>')
      expect(win.jqHelpers('#empty-content .content-body').html()).to.equal('<h3>Not empty now</h3>')
    })
  })
  ...
  it('should append html elements to the selected element', () => {
    cy.window().then(win => {
      expect(win.jqHelpers('#empty-content .content-body').html()).to.equal('')
      win.jqHelpers('#empty-content .content-body').append('<h3>Not empty now</h3>')
      expect(win.jqHelpers('#empty-content .content-body').html()).to.equal('<h3>Not empty now</h3>')
    })
  })
  ...
  it('should serialize form', () => {
    cy.window().then(win => {
      ...
      expect(win.jqHelpers('#contact form').serialize(true)).to.equal('{"name":"value"}')
    })
  })
